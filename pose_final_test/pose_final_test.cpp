...
int main(int argc, char**argv) {
    ...
    for(int objectNr =std::stoi(argv[3]); objectNr<=2;objectNr++) {
        objectIterator=objectNr;
        ...
        for(int RANSACIt=std::stoi(argv[2]); RANSACIt<=4000; RANSACIt=RANSACIt+2000) {
            ...
            for(int sceneNr=std::stoi(argv[1]); sceneNr<=20;sceneNr++) {
                ...
                outfile.open(std::to_string(objectIterator)+filename, std::ios_base::app);
                outfile << std::to_string(RANSACIter)<<","<<std::to_string(sceneNr)<<",";
                outfile.close();
                ...
                double elapsed_secs_global = double(endGlobal - beginGlobal) / CLOCKS_PER_SEC;
                outfile.open(std::to_string(objectIterator)+filename, std::ios_base::app);
                outfile << elapsed_secs_global << ",";
                outfile.close();

                pcl::transformPointCloud (*object, *object_aligned_global, poseGlobal);
                clock_t beginLocal = clock();
                poseLocal=local_allignment(object_aligned_global,scene);
                clock_t endlocal = clock();
                double elapsed_secs_local = double(endlocal - beginLocal) / CLOCKS_PER_SEC;

                clock_t endTotal = clock();
                double elapsed_secs_total = double(endTotal - beginTotal) / CLOCKS_PER_SEC;

                outfile.open(std::to_string(objectIterator)+filename, std::ios_base::app);
                outfile << elapsed_secs_local<<","<<elapsed_secs_total << endl;
                outfile.close();
            }
        }
    }
...
Matrix4f global_allignment(PointCloud<PointT>::Ptr object, PointCloud<PointT>::Ptr scene, int kSearch, float radiusSearch) {
    ...
    */
    ...
    float penalty = FLT_MAX;
    {
        ...
        outfile.open(std::to_string(objectIterator)+filename, std::ios_base::app);
        outfile << rmse << ","<< ((float)inliers/object->size())*100.0<<",";
        outfile.close();
