...
int main(int argc, char**argv) {
    ...
    std::ofstream outfileLocal;
    std::ofstream outfileGlobal;

    outfileLocal.open("triangleRotationLocal.csv", std::ios_base::app);
    outfileGlobal.open("triangleRotationGlobal.csv", std::ios_base::app);
    outfileLocal << "Real Angle, X, Y, Z, Roll, Pitch, Yawn, Error_Roll, Error_Pitch, Error_Yawn" << endl; 
    outfileGlobal << "Real Angle, X, Y, Z, Roll, Pitch, Yawn,Error_Roll, Error_Pitch, Error_Yawn" << endl; 

    int objectIterator=2;
    for(int sceneNr=0; sceneNr<=16;sceneNr++) {
        // Load
        ...
        loadPCDFile("/home/student/Desktop/trianglePCD.pcd", *object);
        loadPCDFile("/home/student/Desktop/ROTATION/triangle"+std::to_string(sceneNr)+".pcd", *scene);
        ...
        cout << "X: "  << angles[0]*180/PI << " Y: " << angles[1]*180/PI << " Z: " << angles[2]*180/PI << endl;
        ...
        std::vector<float> globalAngles = get_Theta(poseGlobal);
        std::vector<float> localAngles = get_Theta(poseGlobal*poseLocal);

        float realAngle =float(sceneNr)*22.5;
        outfileLocal << realAngle << "," << poseLocal(0,3) << "," << poseLocal(1,3) << "," << poseLocal(2,3) << ",";
        outfileLocal << localAngles[0]*180/PI << "," << localAngles[1]*180/PI << "," << localAngles[2]*180/PI << ",";
        outfileLocal << 0-localAngles[0]*180/PI << "," << realAngle-localAngles[1]*180/PI << "," << 0-localAngles[2]*180/PI << endl;

        outfileGlobal << realAngle << "," << poseGlobal(0,3) << "," << poseGlobal(1,3) << "," << poseGlobal(2,3) << ",";
        outfileGlobal << globalAngles[0]*180/PI << "," << globalAngles[1]*180/PI << "," << globalAngles[2]*180/PI << ",";
        outfileGlobal << 0-globalAngles[0]*180/PI << "," << realAngle-globalAngles[1]*180/PI << "," << 0-globalAngles[2]*180/PI << endl;
    }
    return 0;
}
    ...
    */
    ...
    float penalty = FLT_MAX;
    {
        ...
        for(size_t i = 0; i < iter; ++i) {
            ...
            // Update result
            if(penaltyi < penalty && abs(angles[0])<0.0872665&&abs(angles[2])<0.0872665) {
